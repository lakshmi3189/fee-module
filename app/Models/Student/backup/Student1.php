...
class Student extends Model
{
  ...
  //creating for bll - start    
  public function readStudentGroup($req)
  {
    return $getStd = Student::select('id', 'class_id')
      ->where('admission_no', $req->admissionNo)
      ->where('status', '1')
      ->get();
  }
  //creating for bll - end
  ...
  {
    $checkExist = Student::where([['admission_no', '=', $req->admissionNo], ['status', '=', '0']])->count();
    $data = array();
    if ($checkExist > 0) {
